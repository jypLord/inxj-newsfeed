...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    Page<Post> findAllByVisibility(@Param("visibility") Visibility visibility, @Param("pageable") Pageable pageable);
    ...
    Page<Post> findAllFriendVisible(
    ...
    Page<Post> findAllByUserAndVisibility(
        @Param("user") User user, @Param("visibility") Visibility visibility,
        @Param("pageable") Pageable pageable);

    // 유저의 모든 게시글을 생성일 순으로 조회
    @Query("SELECT p FROM Post p WHERE p.user = :user ORDER BY p.createdAt DESC")
    Page<Post> findAllByUser(@Param("user") User user, @Param("pageable") Pageable pageable);
    ...
    Page<Post> findAllByCategoryAndVisibility(
